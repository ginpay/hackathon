...
function setTransaction(data, fromAccount, toAccount, value){
  ...
  var gasPrice = web3.eth.gasPrice * 10;
...
function execRecommend(from_account, to_account, value, recommender, place){
   // データと言われる実行メソッドを指定するためのバイナリ(MethodIDと言われるもの)を作成
   //traveler, recommender, place
   var data = contract.recommend.getData(from_account, recommender, place);
   console.log(data);
   var rawTx = setTransaction(data, from_account, to_account, value);
   sendTransaction(rawTx);
}

function execProofOfVisit(from_account, to_account, value, recommender, place, proof){
   // データと言われる実行メソッドを指定するためのバイナリ(MethodIDと言われるもの)を作成
   //traveler, recommender, place
   var data = contract.recommend.getData(from_account, recommender, place, proof);
   console.log(data);
   var rawTx = setTransaction(data, from_account, to_account, value);
   sendTransaction(rawTx);
}
...
if(process.argv[2] === "deposit"){
  execDepositEth(fromAccount, contractAccount, 1e+18);
  ...
  execRecommend(fromAccount, contractAccount, 0, toAccount, 0x01)
}else if(process.argv[2] === "ProofOfVisit"){
  execProofOfVisit(fromAccount, contractAccount, 0, toAccount, 0x01, 0x01)
}else{
